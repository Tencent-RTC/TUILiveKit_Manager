import { aa as g, ab as d, ac as Ct, R as B, F as qt, ad as et, L as w, a4 as St, a6 as vt, I as l, f as Y } from "./en-US.rgXKsoPj.js";
function us(i, t = 0) {
  const e = Number(i);
...
function bt(i) {
  ...
  if ("RoomType" in i) {
    ...
    return {
      ...
      customInfo: r.CustomInfo ? Xt(r.CustomInfo) : {}
    };
  }
  return s;
}
...
async function A(i) {
  ...
  const s = bt(t.Response.RoomInfo);
...
async function se(i, t, e = 3600, s = "") {
...
async function ds(i, t = 50) {
  return B(`/chat/${i}/messages`, { limit: t });
}
async function fs(i, t) {
  var s;
  const e = ((s = Ct().credentials) == null ? void 0 : s.userId) || "admin";
...
async function de(i, t, e, s) {
  var n;
  const r = s || ((n = Ct().credentials) == null ? void 0 : n.userId) || "admin";
...
class ps {
  constructor(t = {}) {
    this.currentLive = null, this.liveList = [], this.hasMore = !0, this.loading = {}, this.error = null, this.onStateChange = t.onStateChange, this.getActive = t.getActive, this.playerRegistry = new ye({
      playerFactory: t.playerFactory,
      debug: !1
    }), this.userProfileManager = new pe({
      onStateChange: (e) => {
  ...
  setCurrentLive(t) {
    ...
    })) : (this.currentLive = null, this.notifyStateChange({ currentLive: null }), A(t).then((s) => {
      var r;
  ...
  async createLive(t) {
    ...
    try {
      ...
      const r = (await Qt(t)).liveId || t.liveId, n = r ? await A(r) : null;
      if (!n)
        throw new Error("创建直播成功，但获取详情失败");
  ...
  async updateLive(t) {
    ...
    this.setLoading(s, !0), this.setError(null);
    try {
      await te(e, t), this.currentLive && (this.currentLive = { ...this.currentLive, ...t }, this.notifyStateChange({ currentLive: this.currentLive })), this.setLoading(s, !1);
    } catch (n) {
  ...
  async endLive(t) {
    ...
    this.setLoading(s, !0), this.setError(null);
    try {
      await Zt(e), this.liveList = this.liveList.filter((n) => n.liveId !== e), this.currentLive && this.currentLive.liveId === e && (this.currentLive = null), this.notifyStateChange({
        liveList: this.liveList,
        currentLive: this.currentLive
      }), this.setLoading(s, !1);
    } catch (n) {
  ...
  async fetchLiveStats(t) {
    ...
    this.setLoading(r, !0);
    try {
      const o = await ot(e), a = o == null ? void 0 : o.Response, c = a ? this.mergeStatisticData(s, a) : s.stats || {
        liveId: e,
        onlineCount: 0,
        viewCount: 0,
        likeCount: 0,
  ...
  async setAllMute(t, e) {
    if (t)
      try {
        await fe(t, e), this.liveList = this.liveList.map((s) => s.liveId === t ? { ...s, isMuted: e } : s), this.currentLive && this.currentLive.liveId === t && (this.currentLive = { ...this.currentLive, isMuted: e }, this.notifyStateChange({ currentLive: this.currentLive })), this.notifyStateChange({ liveList: this.liveList });
      } catch (s) {
...
function at(i, t = 8e3, e = !1) {
  return new Promise((s, r) => {
    ...
    if (Ie(i)) {
      ...
      Ee(i, t > 8e3 ? t : 15e3).then(() => s(!0)).catch(r);
      return;
    }
...
async function ws(i, t, e = !1) {
  ...
  if (o)
    ...
    try {
      i.type.startsWith("video/") ? await ve(c) : await Se(c);
    } finally {
...
class Ts {
  ...
  async doUrlConfirm(t, e) {
    ...
    if (s) {
      ...
      const n = (async () => {
        try {
          if (await at(s, 8e3, this.skipSvgaValidation), r) return;
  ...
  async _doValidateForSubmit(t) {
    ...
    try {
      return await at(t, 8e3, this.skipSvgaValidation), e || (this.callbacks.onConfirm(t), this.callbacks.setValidating(!1)), t;
    } catch (s) {
...
function Tt(i, t = "image/jpeg", e = 0.92) {
  return new Promise((s, r) => {
    i.toBlob(
      (n) => {
...
async function _s(i, t, e = "image/jpeg", s = 0.92) {
  const r = new Image();
...
async function As(i, t = "image/jpeg", e = 0.92) {
  if (!i)
...
const Gs = {
  field: "createTime",
  direction: "descend"
}, _e = O.liveList, H = "liveList_currentPage", z = "liveList_pageCursors", Ae = 1024;
...
async function Me(i) {
  const { page: t, pageCursors: e, pageSize: s = _e } = i, r = e.get(t) || "", { list: n, next: o } = await Jt({ next: r, count: s, sortDirection: "descend" }), a = n, c = new Map(e);
...
function Ue(i) {
  ...
  try {
    t.setItem(H, String(i.currentPage)), t.setItem(z, JSON.stringify(Array.from(i.pageCursors.entries())));
  } catch {
  }
}
function Oe(i = sessionStorage) {
  let t = 1, e = F();
  try {
    const s = i.getItem(H), r = i.getItem(z);
    if (i.removeItem(H), i.removeItem(z), s && r) {
      const n = Number(s);
      if (n > 0) {
...
const Pe = () => "gift_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9), Ge = (i, t, e) => {
...
async function je(i) {
  ...
  if (t && t.ErrorCode === 0 && t.Response) {
    const e = t.Response.GiftList || [], s = t.Response.CategoryList || [], { giftToCategories: r, giftToCategoryIds: n } = Ve(s), o = e.map((c) => Ge(c, r, n)), a = s.map((c) => Fe(c));
    return {
      gifts: o,
...
class Bs {
  ...
  async fetchGiftList(t) {
    ...
    try {
      const e = await je(t == null ? void 0 : t.language);
  ...
  async updateGift(t) {
    this.setLoading("giftAction", !0), this.setError(null);
    try {
      await Be(t), this.setLoading("giftAction", !1);
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("giftAction", !1), this.setError(s), s;
    }
  ...
  async deleteGift(t) {
    this.setLoading("giftAction", !0), this.setError(null);
    try {
      await We(t), this.giftList = this.giftList.filter((e) => e.id !== t), this.setLoading("giftAction", !1), this.notifyStateChange();
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("giftAction", !1), this.setError(s), s;
    }
  ...
  async createGiftCategory(t) {
    ...
    try {
      await He(t), this.setLoading("categoryAction", !1);
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
  ...
  async updateGiftCategory(t) {
    this.setLoading("categoryAction", !0), this.setError(null);
    try {
      await ze(t), this.setLoading("categoryAction", !1);
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("categoryAction", !1), this.setError(s), s;
    }
  ...
  async deleteGiftCategory(t) {
    this.setLoading("categoryAction", !0), this.setError(null);
    try {
      await qe(t), this.giftCategoryList = this.giftCategoryList.filter((e) => e.id !== t), this.setLoading("categoryAction", !1), this.notifyStateChange();
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("categoryAction", !1), this.setError(s), s;
    }
  ...
  async getGiftLanguage(t) {
    ...
    try {
      const e = await $e(t.giftId, t.language);
  ...
  async setGiftLanguage(t) {
    this.setLoading("giftLanguage", !0), this.setError(null);
    try {
      await Ke(t.giftId, t.language, t.name, t.description), this.setLoading("giftLanguage", !1);
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("giftLanguage", !1), this.setError(s), s;
    }
  ...
  async deleteGiftLanguage(t) {
    this.setLoading("giftLanguage", !0), this.setError(null);
    try {
      await Ye(t.giftId, t.language), this.setLoading("giftLanguage", !1);
    } catch (e) {
      const s = e instanceof Error ? e : new Error(String(e));
      throw this.setLoading("giftLanguage", !1), this.setError(s), s;
    }
  ...
  async setGiftCategoryLanguage(t, e, s, r) {
    this.setLoading("giftCategoryLanguage", !0), this.setError(null);
    try {
      await Je(t, e, s, r), this.setLoading("giftCategoryLanguage", !1);
    } catch (n) {
      const o = n instanceof Error ? n : new Error(String(n));
  ...
  async deleteGiftCategoryLanguage(t, e) {
    this.setLoading("giftCategoryLanguage", !0), this.setError(null);
    try {
      await Ze(t, e), this.setLoading("giftCategoryLanguage", !1);
    } catch (s) {
      const r = s instanceof Error ? s : new Error(String(s));
      throw this.setLoading("giftCategoryLanguage", !1), this.setError(r), r;
    }
...
const ei = 7, ii = 1440 * 60 * 1e3, si = "live-manager-moderation-local-deletions", ri = 1, M = "deletedModerationItems";
let V = {
  ttlDays: ei
}, x = null;
function At(i) {
...
function oi() {
  const i = ni();
  return i ? x || (x = new Promise((t, e) => {
    const s = i.open(si, ri);
    s.onupgradeneeded = () => {
      var o;
      const r = s.result, n = r.objectStoreNames.contains(M) ? (o = s.transaction) == null ? void 0 : o.objectStore(M) : r.createObjectStore(M, { keyPath: "id" });
...
async function U(i, t) {
  ...
  if (e)
    return new Promise((s, r) => {
      const n = e.transaction(M, i), o = n.objectStore(M);
      let a;
      const c = t(o);
      c && (c.onsuccess = () => {
        a = c.result;
...
function Ws(i) {
  i.ttlDays !== void 0 && (V = { ...V, ttlDays: At(i.ttlDays) });
}
function $s() {
  return { ...V };
}
async function ci(i = Date.now()) {
  const e = (await U("readonly", (s) => s.getAll()) || []).filter((s) => s.expiresAt <= i).map((s) => s.id);
  e.length !== 0 && await U("readwrite", (s) => {
...
async function Mt(i, t = {}) {
  ...
  const s = At(t.ttlDays ?? V.ttlDays), r = Date.now(), n = r + s * ii;
  await ci(r), await U("readwrite", (o) => {
    e.forEach((a) => {
      o.put({ id: a, deletedAt: r, expiresAt: n });
    });
  });
}
async function Rt(i = Date.now()) {
  const t = await U("readonly", (r) => r.getAll()), e = [], s = /* @__PURE__ */ new Set();
  return (t || []).forEach((r) => {
    r.expiresAt <= i ? e.push(r.id) : s.add(r.id);
  }), e.length > 0 && await U("readwrite", (r) => {
    e.forEach((n) => r.delete(n));
  }), s;
}
async function li() {
  return (await Rt()).size;
}
async function ui(i) {
  const t = await Rt();
...
function di(i) {
  ...
  if (t.length === 0)
    throw new w("ids 不能为空");
  if (t.length > j)
    throw new w(`ids 不能超过 ${j} 个`);
  return t;
}
function fi(i = 1, t = 20) {
  if (!Number.isInteger(i) || i < 1)
    throw new w("pageNum 必须大于等于 1");
  if (!Number.isInteger(t) || t < 1 || t > j)
    throw new w(`pageSize 必须在 1-${j} 之间`);
}
function hi() {
  return "1";
}
...
function yi(i) {
  ...
  return {
    ...
    type: gi[i.type] || String(i.type) || "unknown",
    createdAtMs: t
  };
}
async function pi(i = {}) {
  ...
  if (!r || r.code !== 0)
    throw new w("getInteractionModerationList failed");
  const n = r.data || { list: [], total: 0, pageNum: t, pageSize: e }, a = (Array.isArray(n.list) ? n.list : []).map(yi);
  return {
    list: a,
...
async function Ks(i) {
  const t = di(i.ids), e = i.items || [];
  if (!i.liveId)
    throw new w("liveId 必填");
...
async function Ys(i) {
  ...
  if (!t)
    throw new w("Content 参数必填");
  await St("/moderation/correction-whitelist", {
    content: t,
    auditType: hi(),
    status: "2"
  });
...
async function Hs() {
  var t;
  const i = await B("/moderation/usage");
...
async function zs(i, t, e, s) {
  var r;
  try {
    const n = { roomIds: i };
    e && (n.startTime = e), s && (n.endTime = s);
    const o = await St("/moderation/room-labels", n);
    ...
    for (const c of o.data.labels) {
      const u = c.roomId;
      if (!u) continue;
      const f = Array.isArray(c.labels) ? c.labels.map((p) => Ii[p] || "unknown") : [];
      a.set(u, f);
    }
    return a;
  } catch (n) {
...
const qs = O.mutedList, Xs = O.bannedList, Ei = 600, Ci = 86400, Dt = (i) => Array.isArray(i) ? i.filter((t) => !!t && typeof t == "object") : [], q = (i, ...t) => {
  const e = t.map((s) => i[s]).find((s) => typeof s == "string" && s.length > 0);
  return typeof e == "string" ? e : "";
}, Ut = (i, ...t) => {
  const e = t.map((r) => i[r]).find((r) => r != null), s = Number(e || 0);
  return Number.isFinite(s) ? s : 0;
}, W = (i) => i.memberAccounts || i.userIds || [];
async function Js(i) {
  var s;
  const t = await ne(i);
  return Dt(((s = t.Response) == null ? void 0 : s.MutedAccountList) || t.MutedAccountList).map((r) => ({
    userId: q(r, "Member_Account", "userId", "UserID"),
...
async function Qs(i) {
  var s;
  const t = await oe(i);
  return Dt(((s = t.Response) == null ? void 0 : s.BannedAccountList) || t.BannedAccountList).map((r) => ({
    userId: q(r, "Member_Account", "userId", "UserID"),
    endTime: Ut(r, "BannedUntil", "endTime"),
    reason: q(r, "Reason", "reason") || void 0
  })).filter((r) => r.userId);
}
function tr(i, t) {
  const e = Math.floor(Date.now() / 1e3);
...
async function er(i) {
  const { liveId: t } = i, e = W(i), s = i.muteTime ?? i.duration ?? Ei;
  await ee(t, e, s);
}
async function ir(i) {
  const { liveId: t } = i, e = W(i);
  await ie(t, e);
}
async function sr(i) {
  const { liveId: t, reason: e = "" } = i, s = W(i), r = i.banTime ?? i.duration ?? Ci;
  await se(t, s, r, e);
}
async function rr(i) {
  const { liveId: t } = i, e = W(i);
...
}, vi = "https://web.sdk.qcloud.com/im/assets/emoji-plugin/", lr = (i) => {
  if (!i)
    return [];
  const t = [];
  let e = i;
...
async function wi(i) {
  ...
  if ((t == null ? void 0 : t.isUrlInputMode) ?? !0)
    try {
      ...
      throw new X(r, "url-validation", o);
    }
  else if (e && t)
    try {
      return await t.uploadPendingFile() || "";
    } catch (o) {
      throw new X(r, "upload", o);
    }
  else
    return s.trim();
}
async function yr(i) {
  return Promise.all(i.map(wi));
}
function pr(i, t = "图片") {
  if (i instanceof X)
    return i.message;
...
async function vr() {
  try {
    return !!(await xi()).enabled;
  } catch {
    return !1;
  }
}
async function br(i, t = "cover", e) {
...
const Gi = "live_", Ar = 43, Mr = 100, Fi = "VideoDynamicGrid9Seats", Vi = 8, G = 9 / 16, Pt = 16 / 9, E = {
  maxCount: 10,
  maxKeyBytes: 50,
...
function Wi(i) {
  return ji.find((t) => t.value === i);
}
function Ur(i) {
  var t;
  return ((t = Wi(i)) == null ? void 0 : t.orientation) === "landscape" ? Pt : G;
}
function Or(i) {
...
function Nr(i) {
  ...
  for (const r of i) {
    ...
    const o = T(n);
    if (o > E.maxKeyBytes)
      return { type: "key-too-long", key: n, max: E.maxKeyBytes, current: o };
    const a = T(r.value);
    if (a > E.maxValueBytes)
      return { type: "value-too-long", key: n, max: E.maxValueBytes, current: a };
    e += a, t[n] = r.value;
  }
...
function Pr(i) {
  const { formData: t, coverUrl: e, customInfos: s, useObsStreaming: r } = i, n = $i(s), o = {
    liveId: ki(t.anchorId),
    anchorId: t.anchorId,
    title: t.liveName,
    seatTemplate: t.seatTemplate
  };
  return e && (o.coverUrl = e), Bi(t.seatTemplate) && (o.maxSeatCount = t.maxSeatCount), Object.keys(n).length > 0 && (o.customInfo = n), r && (o.useObsStreaming = !0), o;
}
function Ki(i) {
  if (!i) return [];
  if (typeof i == "string")
    try {
      return Ki(JSON.parse(i));
    } catch {
      return [];
    }
...
async function Z(i, t) {
  const [e, s] = await Promise.all([
    ...
    ce(i).then((r) => {
      var n;
      return Hi((n = r.Response) == null ? void 0 : n.SeatList);
...
async function Fr(i) {
  const { liveId: t, anchorId: e, onStatusChange: s, messages: r = {} } = i, n = Gt(e), o = r.getStreamInfoFailed || "Get Stream Info Failed";
  let a = null, c = "";
  try {
    const u = await et(t, n);
    a = Ft(u), a || (c = o);
  } catch (u) {
    c = (u instanceof Error ? u.message : "") || o;
  }
  try {
    const { hasRobot: u, onSeat: f } = await Z(t, n);
    let p = f;
    if (!u) {
      s == null || s("creating");
      const h = await ge(n, `OBS Robot ${n}`);
      if (h.ErrorCode !== 0 && h.Error !== 0 && h.ErrorCode !== Yi)
        throw new Error(h.ErrorInfo || r.importAccountFailed || "Import Account Failed");
      const _ = await ue(t, [n]);
      if (_.ErrorCode !== 0)
        throw new Error(_.ErrorInfo || r.addRobotFailed || "Add Robot Failed");
      p = (await Z(t, n)).onSeat;
    }
    if (!p) {
      s == null || s("seating");
      const h = await le(t, n);
      if (h.ErrorCode !== 0)
...
async function zi(i) {
  const { liveId: t, anchorId: e, getStreamInfoFailedMessage: s = "Get Stream Info Failed" } = i, r = Gt(e);
  try {
    const { hasRobot: n, onSeat: o } = await Z(t, r);
    if (!n || !o)
      return {
        robotId: r,
        hasRobot: n,
...
function $(i, t) {
  ...
  return document.body.removeChild(r), Q.set(e, n), n;
}
function Xi() {
  Q.clear();
}
function jt(i) {
  const t = i.roomIdText ?? i.liveIdText ?? "", {
    ...
    minVisibleTags: c = 2,
    maxVisibleTags: u = 99
  } = i, p = $("live-id-badge", t), h = r + p, _ = e - n, C = s.map(
    (m) => $("live-card-tag-overlay", m)
  ), N = $("live-card-tag-overlay live-card-tag-more", "...");
  let S = 0, I = 0;
  for (let m = 0; m < C.length; m++) {
    const y = C[m], st = C.length - m - 1, Yt = st > 0 ? N : 0, rt = I > 0 ? o : 0, Ht = S + rt + y + (st > 0 ? o + Yt : 0), nt = _ - Ht, zt = Math.max(60, nt - r - a);
    if (p > zt || nt < h + a)
      break;
    S += rt + y, I++;
  }
  if (C.length - I > 0 && (S += o + N), s.length >= c && I < c) {
    let m = 0;
    for (let y = 0; y < c; y++)
      m += (y > 0 ? o : 0) + C[y];
    s.length > c && (m += o + N), I = c, S = m;
  }
  if (I > u) {
    I = u;
    let m = 0;
    for (let y = 0; y < u; y++)
      m += (y > 0 ? o : 0) + C[y];
    s.length > u && (m += o + N), S = m;
  }
  const $t = _ - S, Kt = Math.max(
    60,
    $t - r - a
  );
  return {
    visibleCount: I,
    showMore: s.length > I,
    idMaxWidth: `${Math.round(Kt)}px`,
    tagsMaxWidth: `${Math.round(S)}px`
  };
}
function zr(i, t, e, s, r) {
  return i.map((n) => {
    const o = t(n), a = e(n);
    return jt({
      containerWidth: r,
...
const L = {
  ...
  "zh-Hant": "tc",
  en: "en"
}, Xr = ["sc", "tc", "en"], Jr = 20, Zr = 20, Qr = 20, tn = 100, dt = 20, en = 45;
function kt(i) {
  return new TextEncoder().encode(i).length;
}
function sn(i, t) {
...
function ht(i) {
  ...
  return t.forEach((r) => {
    ...
    const n = Y(r.languageList, "name", "description"), o = (r.categoryIds || []).map((a) => e.get(String(a)) || a);
    return {
      ...r,
      name: (n == null ? void 0 : n.name) || r.defaultName || r.name,
      description: (n == null ? void 0 : n.description) || r.defaultDescription || r.description,
...
const cn = 10, ln = 20, un = 20, gn = 60, tt = "gift_categories_cache";
function es(i) {
  return i.map((t) => {
...
function is(i, t = sessionStorage) {
  if (i.length !== 0)
    try {
      t.setItem(tt, JSON.stringify(i));
    } catch {
    }
}
function ss(i = sessionStorage) {
  try {
    const t = i.getItem(tt);
    return t ? (i.removeItem(tt), es(JSON.parse(t))) : [];
  } catch {
    return [];
  }
}
...
class mn {
  ...
  setPreview(t) {
    ...
    const e = we(this.currentUrl || void 0, t);
   ...
   */
  clearPreview() {
    this.currentUrl && (_t(this.currentUrl), this.currentUrl = "");
  }
  /**
...
function yn(i) {
  const t = [
    { key: "liveId", i18nKey: l.LIVE_ID, className: "col-id", minWidth: 80, maxWidth: v.LIVE_ID_MAX_WIDTH, fitContent: !0, flexible: !0 },
    { key: "liveName", i18nKey: l.LIVE_TITLE, className: "col-title", minWidth: 120, maxWidth: v.LIVE_TITLE_MAX_WIDTH, fitContent: !0, flexible: !0, ellipsis: !0 },
    { key: "coverUrl", i18nKey: l.COVER_IMAGE, className: "col-cover", width: v.COVER_WIDTH },
    { key: "anchor", i18nKey: l.ANCHOR_ID, className: "col-anchor", minWidth: 80, maxWidth: v.ANCHOR_ID_MAX_WIDTH, fitContent: !0, flexible: !0 },
    { key: "createdAt", i18nKey: l.CREATE_TIME, className: "live-list-col-time", width: v.CREATED_AT_WIDTH }
  ];
  return i.hasExtraColumn && t.push({ key: "customer-extra", className: "live-list-col-customer-extra", width: v.EXTRA_COLUMN_WIDTH }), t.push({ key: "actions", i18nKey: l.ACTIONS, className: "live-list-col-action", maxWidth: v.ACTIONS_MAX_WIDTH, fitContent: !0 }), t;
}
function pn(i) {
...
async function ns(i, t) {
  var n;
  const e = ((n = i.anchor) == null ? void 0 : n.userId) || "", s = await zi({
    liveId: i.liveId,
    anchorId: e,
...
class Ln {
  constructor(t) {
    this.opts = t, this.state = {
      ...
      editingLive: null
    }, this.listeners = /* @__PURE__ */ new Set(), this.pageCursors = F(), this.destroyed = !1, this.getState = () => this.state, this.subscribe = (e) => (this.listeners.add(e), () => {
  ...
  async loadPage(t) {
    ...
    try {
      ...
      e instanceof ct ? this.opts.toast.error(lt(e.errorCode, e.errorInfo, s)) : this.opts.toast.error(s);
    } finally {
  ...
  isSearchInputTooLong() {
    return Ne(this.state.searchInput, Ae);
  }
  async search() {
    ...
    if (t) {
      ...
      try {
        ...
        if (console.error("搜索直播失败:", e), e instanceof ct)
          this.opts.toast.error(lt(
            e.errorCode,
            e.errorInfo,
  ...
  async clearSearch() {
    this.pageCursors = F(), this.setState({
      searchInput: "",
      currentPage: 1,
      hasMoreData: !0
  ...
  async showDetail(t) {
    ...
    const { snapshot: e, toastError: s } = await ns(t, this.opts.t);
  ...
  async confirmDelete() {
    ...
    if (t)
      try {
        ...
        this.setState({ confirmDialog: { ...yt } });
      }
  }
  // -------- 创建 / 编辑弹窗 --------
  openCreateModal() {
...
class In {
  ...
  async fetchGiftList() {
    this.setState({ loading: !0 });
    try {
      const t = await this.opts.actions.fetchGiftList(), e = t.giftCategoryList || [], { gifts: s, categories: r } = ht({
        gifts: t.giftList || [],
        categories: e
      });
      this.setState({
        giftList: s,
        displayList: P(s, this.state.searchInput),
  ...
  onLanguageChanged() {
    ...
    this.setState({
      giftList: t,
      categoryList: e,
      displayList: P(t, this.state.searchInput)
    });
  }
  // -------- 搜索 --------
  setSearchInput(t) {
    const e = String(t ?? "");
    this.setState({ searchInput: e }), !ft(e, dt) && this.setState({ displayList: P(this.state.giftList, e) });
  }
  isSearchInputTooLong() {
    return ft(this.state.searchInput, dt);
  }
  ...
  search() {
    ...
    const e = P(this.state.giftList, this.state.searchInput);
  ...
  goToCategoryManagement() {
    is(this.state.categoryList), this.opts.onNavigateToCategoryManagement();
  }
  async copyGiftId(t) {
    await it(String(t || "")), this.opts.toast.success(this.opts.t(l.COPY_LABEL_COPIED, { label: this.opts.t(l.GIFT_ID) }));
  }
  ...
  async openGiftLangConfigDialog(t) {
    const e = this.state.giftList.find((n) => n.id === t), { config: s, langsToFetch: r } = Zi(e == null ? void 0 : e.languageList);
    if (this.setState({
      editingId: t,
      giftLangConfig: s,
      langConfigVisible: !0
  ...
  async openLangEditDialog(t, e) {
    const s = this.state.giftList.find((o) => o.id === t), r = Qi(s == null ? void 0 : s.languageList, e);
    let n = r.form;
    if (r.needsFetch)
      try {
...
class En {
   ...
   */
  onLanguageChanged() {
    this.state.categoryList.length !== 0 && this.setState({ categoryList: rs(this.state.categoryList) });
  }
  // -------- 导航 / 复制 --------
  goBack() {
  ...
  async copyCategoryId(t) {
    await it(String(t || "")), this.opts.toast.success(
  ...
  async openLangConfigDialog(t) {
    const e = this.state.categoryList.find((n) => n.id === t), s = Et(Lt), r = [];
  ...
  async openLangEditDialog(t, e) {
    var a;
    const s = this.state.categoryList.find((c) => c.id === t), r = L[e].code, n = (a = s == null ? void 0 : s.languageList) == null ? void 0 : a.find((c) => K(c).langCode === r);
    let o = { ...It };
    if (n) {
      const c = K(n);
...
function K(i) {
  if (typeof i == "string")
    return { langCode: i, langKey: gt[i], name: "", description: "" };
  const t = i.language || "";
  return {
    langCode: t,
    langKey: t ? gt[t] : void 0,
    name: i.categoryName || "",
