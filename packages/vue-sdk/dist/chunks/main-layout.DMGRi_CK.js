import { t as ct, c as y, h as C, r as A } from "./logger.DfjyL4S7.js";
import { at as u, au as d, L as v, a6 as lt, M as o, b as At, ax as Y, ap as Dt, af as Tt, a5 as _t, a8 as Mt, f as G } from "./en-US.BxRaQHCQ.js";
function ls(s, t = 0) {
  const e = Number(s);
...
async function wt(s = {}) {
  const t = s.next || "0", e = s.count || 20, i = await u(d.fetchLiveList, {
    Next: t,
    Count: e,
...
async function D(s) {
  const [t, e] = await Promise.all([
    u(d.getRoomInfo, { RoomId: s }),
    u(d.getRoomMetadata, { RoomId: s, Keys: [] })
  ]);
...
async function z(s) {
  return u(d.getLiveStatistic, { RoomId: s });
}
async function Ot(s) {
  return u(d.destroyRoom, { RoomId: s });
}
async function Pt(s) {
  ...
  if (s.useObsStreaming && (e.KeepOwnerOnSeat = !1), s.liveName && s.liveName.trim() && (e.RoomName = s.liveName), s.coverUrl && s.coverUrl.trim() && (e.CoverURL = s.coverUrl.trim()), s.maxSeatCount && (s.seatTemplate === "AudioSalon" || s.seatTemplate === "Karaoke") && (e.MaxSeatCount = Number(s.maxSeatCount)), await u(d.createLive, { RoomInfo: e }), s.customInfo && Object.keys(s.customInfo).length > 0) {
    const r = Object.entries(s.customInfo).map(([a, n]) => ({ Key: a, Value: n }));
    await u(d.setRoomMetadata, { RoomId: t, Metadata: r });
  }
  const i = await D(t);
  if (!i)
    throw new v("");
...
async function Nt(s, t) {
  ...
  if (t.liveName !== void 0 && (i.RoomName = t.liveName), t.coverUrl !== void 0 && (i.CoverURL = t.coverUrl), t.isMessageDisabled !== void 0 && (i.IsMessageDisabled = t.isMessageDisabled), e.push(u(d.updateLiveInfo, { RoomInfo: i })), t.customInfo && Object.keys(t.customInfo).length > 0) {
    const a = Object.entries(t.customInfo).map(([n, c]) => ({ Key: n, Value: c }));
    e.push(u(d.setRoomMetadata, { RoomId: s, Metadata: a }));
  }
  return t.deleteKeys && t.deleteKeys.length > 0 && e.push(u(d.delRoomMetadata, { RoomId: s, Keys: t.deleteKeys })), (await Promise.all(e))[0];
}
async function Ft(s, t, e = 600) {
  return u(d.muteMember, { RoomId: s, MemberList_Account: t, MuteTime: e });
}
async function Ut(s, t) {
  return u(d.muteMember, { RoomId: s, MemberList_Account: t, MuteTime: 0 });
}
async function Gt(s, t, e = 3600, i = "") {
  return u(d.banMember, { RoomId: s, MemberList_Account: t, Duration: e, Reason: i });
}
async function xt(s, t) {
  return u(d.unbanMember, { RoomId: s, MemberList_Account: t });
}
async function Vt(s, t = {}) {
  return u(d.getMutedMemberList, {
    RoomId: s,
    Next: t.next,
    Count: t.count,
    Offset: t.offset,
...
async function kt(s, t = {}) {
  return u(d.getBannedMemberList, {
    RoomId: s,
    Next: t.next,
    Count: t.count,
    Offset: t.offset,
...
async function Bt(s) {
  return u(d.getRobot, { RoomId: s });
}
async function Kt(s) {
  return u(d.getSeatList, { RoomId: s });
}
async function $t(s, t, e = 0) {
  return u(d.pickUserOnSeat, { RoomId: s, Member_Account: t, Index: e });
}
async function qt(s, t) {
  return u(d.addRobot, { RoomId: s, RobotList_Account: t });
}
async function Yt(s, t, e) {
  return u(d.importAccount, { UserID: s, Nick: t || "", FaceUrl: "" });
}
async function ds(s, t = 50) {
  return lt(`/chat/${s}/messages`, { limit: t });
}
async function hs(s, t) {
  const e = ct().credentials?.userId || "admin";
  return u(d.sendTextMsg, {
    RoomId: s,
    Sender_Account: e,
    TextContent: t,
    MsgPriority: "Normal"
...
async function Ht(s, t, e, i) {
  const r = i || ct().credentials?.userId || "admin";
  return u(d.sendCustomMsg, {
    RoomId: s,
    Sender_Account: r,
    BusinessId: t,
    Data: e
...
async function gs(s, t, e) {
  return u(d.muteMember, {
    RoomId: s,
...
class Xt {
   ...
   */
  setPlayerFactory(t) {
    p.info("PlayerRegistry", "setPlayerFactory called", typeof t), this.playerFactory = t;
  }
  /**
   * 开始播放
   * @param liveId 直播 ID
  ...
  async startPlay(t) {
    ...
    if (!this.playerFactory)
      throw p.error("PlayerRegistry", "playerFactory is not configured!"), new Error(
        "playerFactory is not configured. Please provide it in ServerConfig or call setPlayerFactory()."
      );
    this.debug && p.info("PlayerRegistry", "startPlay called", { liveId: e, containerId: i.id });
    try {
      if (this.stoppingSet.has(e)) {
        this.debug && p.info("PlayerRegistry", `Waiting for stopPlay to complete for ${e}`);
        const n = this.stoppingPromiseMap.get(e);
        n && await n;
      }
      this.playerMap.has(e) && await this.stopPlay(e);
      const a = this.playerFactory(e);
      this.playerMap.set(e, a), await a.play(i, r), this.debug && p.info("PlayerRegistry", `Started playing live ${e}`);
    } catch (a) {
  ...
  async stopPlay(t) {
    ...
    if (this.stoppingSet.has(t)) {
      this.debug && p.warn("PlayerRegistry", `stopPlay already in progress for ${t}, skipping`);
    ...
    const e = (async () => {
      try {
        ...
        i && (await i.stop(), await i.destroy(), this.playerMap.delete(t), this.debug && p.info("PlayerRegistry", `Stopped playing live ${t}`));
      } catch (i) {
        this.debug && p.warn("PlayerRegistry", `Error stopping live ${t}:`, i);
      } finally {
  ...
  async stopAll() {
    ...
    await Promise.allSettled(t), this.debug && p.info("PlayerRegistry", "Stopped all players");
  }
  /**
   * 销毁注册表（清理所有资源）
   */
  async destroy() {
    await this.stopAll(), this.playerFactory = null, this.debug && p.info("PlayerRegistry", "Destroyed");
  }
  /**
   * 获取播放器状态快照（用于调试）
   */
...
class Zt {
  ...
  async fetchProfiles(t) {
    ...
    try {
      ...
      throw this.setLoading(e, !1), Jt.error("UserProfileManager", "fetchProfiles failed:", i), i;
    }
  }
  /** 获取需要的用户ID（过滤已缓存的） */
  getMissingUserIds(t) {
...
class ps {
  constructor(t = {}) {
    this.currentLive = null, this.liveList = [], this.hasMore = !0, this.loading = {}, this.error = null, this.onStateChange = t.onStateChange, this.getActive = t.getActive, this.playerRegistry = new Xt({
      ...
      debug: !1
    }), this.userProfileManager = new Zt({
      onStateChange: (e) => {
  ...
  // ========= init =========
  init(t) {
    f.info("init", "init called", {
      hasPlayerFactory: !!t.playerFactory,
      baseURL: t.baseURL
    }), t.playerFactory ? (f.info("LiveMonitorCore", "Setting playerFactory"), this.playerRegistry.setPlayerFactory(t.playerFactory)) : f.warn("LiveMonitorCore", "No playerFactory provided!");
  }
  // ========= 直播列表操作 =========
  ...
  async fetchLiveDetail(t) {
    ...
    try {
      const [r, a, n] = await Promise.allSettled([
        D(e),
        z(e),
      ...
      if (r.status === "fulfilled")
        ...
        throw r.reason;
      const l = a.status === "fulfilled" ? a.value?.Response : null;
      return c && n.status === "fulfilled" && n.value && (c.streamInfo = n.value), c && l && (c.stats = this.mergeStatisticData(c, l)), c && (this.liveList.some((g) => g.liveId === c.liveId) ? this.liveList = this.liveList.map(
        (g) => g.liveId === c.liveId ? c : g
      ) : this.liveList = [c, ...this.liveList], this.currentLive = c), this.setLoading(i, !1), this.notifyStateChange({
        liveList: this.liveList,
        currentLive: this.currentLive
      }), c;
    } catch (r) {
      const a = r instanceof Error ? r : new Error(String(r));
      throw this.setError(a), this.setLoading(i, !1), f.error("LiveMonitorCore", "fetchLiveDetail failed:", a), a;
    }
  }
  ...
  async fetchPushInfo(t) {
    try {
      ...
      return f.error("LiveMonitorCore", "fetchPushInfo failed:", e), null;
    }
  }
  // ========= 创建直播 =========
  async createLive(t) {
    ...
    try {
      ...
      const r = (await Pt(t)).liveId || t.liveId, a = r ? await D(r) : null;
      if (!a)
      ...
      throw this.setError(r), this.setLoading(e, !1), f.error("LiveMonitorCore", "createLive failed:", r), r;
    }
  }
  // ========= 更新直播 =========
  async updateLive(t) {
    ...
    try {
      ...
      const a = r instanceof Error ? r : new Error(String(r));
      throw this.setError(a), this.setLoading(i, !1), f.error("LiveMonitorCore", "updateLive failed:", a), a;
    }
  }
  // ========= 结束直播 =========
  async endLive(t) {
    ...
    try {
      ...
      const a = r instanceof Error ? r : new Error(String(r));
      throw this.setError(a), this.setLoading(i, !1), f.error("LiveMonitorCore", "endLive failed:", a), a;
    }
  }
  /** 获取缓存的用户资料 */
  getUserProfile(t) {
  ...
  async fetchLiveStats(t) {
    ...
    try {
      ...
      throw this.setError(n), this.setLoading(r, !1), f.error("LiveMonitorCore", "fetchLiveStats failed:", n), n;
    }
  }
  // ========= 播放控制 =========
  async startPlay(t) {
  ...
  async setAllMute(t, e) {
    if (t)
      try {
        await Wt(t, e), this.liveList = this.liveList.map((i) => i.liveId === t ? { ...i, isMuted: e } : i), this.currentLive && this.currentLive.liveId === t && (this.currentLive = { ...this.currentLive, isMuted: e }, this.notifyStateChange({ currentLive: this.currentLive })), this.notifyStateChange({ liveList: this.liveList });
      } catch (i) {
        const r = i instanceof Error ? i : new Error(String(i));
        throw this.setError(r), f.error("LiveMonitorCore", "setAllMute failed:", r), r;
      }
  }
  /**
   * 从服务器获取全员禁言状态
  ...
  async fetchMuteStatus(t) {
    if (t)
      try {
        ...
        this.setError(i), f.error("LiveMonitorCore", "fetchMuteStatus failed:", i);
      }
  }
  // ========= 重置状态 =========
  reset() {
...
const b = y("PaginatedList");
function j() {
  return /* @__PURE__ */ new Map([[1, ""]]);
}
class ys {
  constructor(t) {
    this.hasMore = !0, this.pageData = [], this.loading = !1, this.isLoading = !1, this.listeners = /* @__PURE__ */ new Set(), this.pageSize = t.pageSize ?? 8, this.pageCursors = t.initialPageCursors ?? j(), this.currentPage = t.initialPage ?? 1, this.fetchPageFn = t.fetchPage;
  }
  // -------- 订阅 / 状态 --------
  getSnapshot() {
    return {
  ...
  notify() {
    ...
    b.info("PaginatedListController", "notify called, listeners:", this.listeners.size, ", snapshot:", { pageDataLength: t.pageData.length, loading: t.loading, currentPage: t.currentPage }), this.listeners.forEach((e) => e(t));
  }
  // -------- 防止并发 --------
  ...
  async goToFirstPage() {
    if (this.isLoading) {
      b.warn("PaginatedListController", "goToFirstPage: already loading, skipping");
      return;
    }
    this.pageCursors = j(), this.currentPage = 1, await this.loadPage();
  }
  /**
   * 上一页
   */
  async prevPage() {
    if (this.isLoading) {
      b.warn("PaginatedListController", "prevPage: already loading, skipping");
      return;
    }
  ...
  async nextPage() {
    if (this.isLoading) {
      b.warn("PaginatedListController", "nextPage: already loading, skipping");
      return;
    }
  ...
  async refreshCurrentPage() {
    if (this.isLoading) {
      b.warn("PaginatedListController", "refreshCurrentPage: already loading, skipping");
      return;
    }
    await this.loadPage();
  }
  ...
  async loadPage() {
    ...
    try {
      ...
      b.error("PaginatedListController", "loadPage failed:", t), this.pageData = [], this.hasMore = !1;
    } finally {
...
function ee(s) {
  try {
    ...
    Qt.warn("general", "Failed to revoke ObjectURL:", s, t);
  }
}
function bs(s, t) {
  return s && ee(s), te(t);
...
const Os = {
  field: "createTime",
  direction: "descend"
}, ie = _.liveList, V = "liveList_currentPage", k = "liveList_pageCursors", re = 1024;
function w() {
  return /* @__PURE__ */ new Map([[1, ""]]);
}
async function ae(s) {
  const { page: t, pageCursors: e, pageSize: i = ie } = s, r = e.get(t) || "", { list: a, next: n } = await wt({ next: r, count: i, sortDirection: "descend" }), c = a, l = new Map(e);
  ...
  return r.sort((n, c) => {
    ...
    switch (e) {
      ...
      case "viewerCount":
        ...
        break;
      case "duration": {
        const h = n.endedAt ? n.endedAt - n.createdAt : Date.now() / 1e3 - n.createdAt, g = c.endedAt ? c.endedAt - c.createdAt : Date.now() / 1e3 - c.createdAt;
        l = h - g;
        break;
      }
      case "status": {
        const h = n.endedAt ? 1 : 0, g = c.endedAt ? 1 : 0;
        l = h - g;
        break;
      }
      default:
...
const Gs = _.mutedList, xs = _.bannedList, he = 600, ge = 86400, dt = (s) => Array.isArray(s) ? s.filter((t) => !!t && typeof t == "object") : [], B = (s, ...t) => {
...
async function Vs(s) {
  ...
  return dt(t.Response?.MutedAccountList || t.MutedAccountList).map((i) => ({
    userId: B(i, "Member_Account", "userId", "UserID"),
...
async function Bs(s) {
  ...
  return dt(t.Response?.BannedAccountList || t.BannedAccountList).map((i) => ({
    ...
    reason: B(i, "Reason", "reason") || void 0
  })).filter((i) => i.userId);
}
function Ks(s, t) {
  const e = Math.floor(Date.now() / 1e3);
...
async function $s(s) {
  const { liveId: t } = s, e = P(s), i = s.muteTime ?? s.duration ?? he;
  await Ft(t, e, i);
}
async function qs(s) {
...
async function Ys(s) {
  const { liveId: t, reason: e = "" } = s, i = P(s), r = s.banTime ?? s.duration ?? ge;
  await Gt(t, i, r, e);
}
async function Hs(s) {
  const { liveId: t } = s, e = P(s);
...
const me = {
  ...
  [-2105]: "Invalid room ID. Custom IDs must be printable ASCII (0x20-0x7e), max 48 bytes",
  [-2107]: "Invalid room name. Max 30 bytes, UTF-8 encoding required",
  [-2108]: "You are already in another room, please leave first"
}, Ee = {
  [-2200]: "未找到该用户"
}, ve = {
  [-2200]: "User not found"
  ...
  [-2380]: "All-member mute is enabled in this room",
  [-2381]: "You have been muted"
}, Ae = {
  [-4001]: "当前房间不支持预加载"
}, De = {
  [-4001]: "Room preloading is not supported"
}, Te = {
  ...
  100003: "Room ID is already in use, please choose another",
  100004: "Room not found or already dismissed",
  100005: "Not a room member",
  100006: "Insufficient permissions",
  100007: "No payment info, please purchase a package in the console",
  100008: "Room members are full",
  100009: "Tag count exceeds limit",
  100010: "Room ID already in use, but you are the owner",
  100011: "Room ID is occupied by IM, please choose another or dismiss the IM group first",
  ...
  100018: "Room password required",
  100019: "Incorrect room password",
  ...
  100211: "This room does not support co-hosting",
  100251: "Co-host list is empty",
  100400: "Connection not found or already ended",
  100401: "Room is already connected",
  100402: "Room has a pending connection request",
  100403: "Room is currently connected to another room",
  ...
  100412: "No valid rooms in this Battle",
  100413: "Battles too frequent, please try again later",
  100414: "Room is no longer in a Battle",
  100415: "Room is in another Battle",
  100416: "Room has a pending Battle request",
  100419: "Room is in a Battle",
  100420: "Battle has not started yet",
  100421: "Battle has ended",
  100500: "Room meta key count exceeds limit",
  100501: "Room meta value for a single key exceeds max byte limit",
  100502: "Room meta total value size exceeds max byte limit",
  100503: "None of the keys to delete exist in room meta",
  100504: "Room meta key size exceeds max byte limit"
}, we = {
  ...me,
  ...ye,
  ...Ie,
...
function X(s, t, e, i) {
  if (t)
    return t;
  const r = i ?? fe(), a = r === "en-US" ? Oe : we, n = e ?? (r === "en-US" ? `Unknown error (code: ${s})` : `未知错误（错误码: ${s}）`);
  return a[s] ?? n;
}
function ri(s) {
  return s === 0;
...
const Ne = "live_", gi = 43, fi = 100, Fe = "VideoDynamicGrid9Seats", Ue = 8, R = 9 / 16, Lt = 16 / 9, I = {
  maxCount: 10,
  maxKeyBytes: 50,
...
function ke(s) {
  return Ge.find((t) => t.value === s);
}
function yi(s) {
  return ke(s)?.orientation === "landscape" ? Lt : R;
}
function Li(s) {
...
function Ei(s) {
  const { formData: t, coverUrl: e, customInfos: i, useObsStreaming: r } = s, a = Be(i), n = {
    liveId: xe(t.anchorId),
    anchorId: t.anchorId,
    title: t.liveName,
    seatTemplate: t.seatTemplate
  };
  return e && (n.coverUrl = e), Ve(t.seatTemplate) && (n.maxSeatCount = t.maxSeatCount), Object.keys(a).length > 0 && (n.customInfo = a), r && (n.useObsStreaming = !0), n;
}
function Ke(s) {
  if (!s) return [];
  if (typeof s == "string")
    try {
      return Ke(JSON.parse(s));
    } catch {
      return [];
    }
...
function vi(s) {
  return new Promise((t) => {
    ...
    e.crossOrigin = "anonymous", e.onload = () => {
      ...
      t(i > 1 ? Lt : R);
    }, e.onerror = () => t(R), e.src = s;
  });
}
const $e = y("obs"), qe = 70102;
function It(s) {
  return `live_obs_${s}`;
}
function Ye(s) {
...
async function $(s, t) {
  const [e, i] = await Promise.all([
    Bt(s).then((r) => r.Response?.RobotList_Account || []).catch(() => []),
    Kt(s).then((r) => Ye(r.Response?.SeatList)).catch(() => /* @__PURE__ */ new Set())
  ]);
  return {
...
async function Ci(s) {
  const { liveId: t, anchorId: e, onStatusChange: i, messages: r = {} } = s, a = It(e), n = r.getStreamInfoFailed || "Get Stream Info Failed";
  let c = null, l = "";
  try {
    const { hasRobot: h, onSeat: g } = await $(t, a);
    let W = g;
    if (!h) {
      i?.("creating");
      const L = await Yt(a, `OBS Robot ${a}`);
      if (L.ErrorCode !== 0 && L.Error !== 0 && L.ErrorCode !== qe)
        throw new v(L.ErrorInfo || "", L.ErrorCode);
      const N = await qt(t, [a]);
      if (N.ErrorCode !== 0)
        throw new v(N.ErrorInfo || "", N.ErrorCode);
      W = (await $(t, a)).onSeat;
    }
    if (!W) {
      i?.("seating");
    ...
    $e.error("OBS setup failed", "", h);
    const g = (h instanceof Error ? h.message : "") || r.setupFailed || "OBS Setup Failed";
    return i?.("error"), {
      robotId: a,
      streamInfo: c,
      streamInfoError: l,
  ...
  try {
    ...
    l = (h instanceof Error ? h.message : "") || n;
  }
  return {
    robotId: a,
    streamInfo: c,
...
async function He(s) {
  const { liveId: t, anchorId: e, getStreamInfoFailedMessage: i = "Get Stream Info Failed" } = s, r = It(e);
  try {
    const { hasRobot: a, onSeat: n } = await $(t, r);
    if (!a || !n)
      return {
        robotId: r,
        hasRobot: a,
...
const m = {
  ...
  "zh-Hant": "tc",
  en: "en"
}, Oi = ["sc", "tc", "en"], Pi = 20, Ni = 20, Fi = 20, Ui = 100, Q = 50, Gi = 45;
function vt(s) {
  return new TextEncoder().encode(s).length;
}
function xi(s, t) {
...
function et(s) {
  const t = s.categories.map((r) => {
    const a = G(r.languageList, "categoryName", "categoryDescription");
    return {
      ...r,
  ...
  return t.forEach((r) => {
    ...
    const a = G(r.languageList, "name", "description"), n = (r.categoryIds || []).map((c) => e.get(String(c)) || c);
    return {
      ...r,
      name: a?.name || r.defaultName || r.name,
      description: a?.description || r.defaultDescription || r.description,
...
function Qe(s, t = sessionStorage) {
  if (s.length !== 0)
    try {
...
function ts(s = sessionStorage) {
  try {
    const t = s.getItem(q);
    return t ? (s.removeItem(q), Ze(JSON.parse(t))) : [];
  } catch {
    return [];
  }
}
function es(s) {
  return s.map((t) => {
    const e = G(t.languageList, "categoryName", "categoryDescription");
    return {
      ...t,
...
async function ss(s, t) {
  const e = s.anchor?.userId || "", i = await He({
    liveId: s.liveId,
    anchorId: e,
    getStreamInfoFailedMessage: t(o.GET_STREAM_INFO_FAILED)
  }), r = {
    visible: !0,
    live: s,
    robotId: i.robotId,
  ...
  return i.status === "ready" ? {
    ...
    snapshot: {
      ...r,
      robotStatus: "none",
      robotStatusText: t(o.OBS_SETUP_FAILED)
    }
  } : {
    snapshot: {
      ...r,
      robotStatus: "error",
      robotStatusText: t(o.REQUEST_FAILED, {
        error: i.errorMessage || t(o.NETWORK_ERROR)
      })
    }
  };
}
...
class Yi {
  constructor(t) {
    this.opts = t, this.state = {
      ...
      editingLive: null
    }, this.listeners = /* @__PURE__ */ new Set(), this.pageCursors = w(), this.destroyed = !1, this.getState = () => this.state, this.subscribe = (e) => (this.listeners.add(e), () => {
  ...
  async loadPage(t) {
    ...
    try {
      ...
      e instanceof v ? this.opts.toast.error(X(e.code ?? 0, e.message, i)) : this.opts.toast.error(i);
    } finally {
  ...
  isSearchInputTooLong() {
    return ue(this.state.searchInput, re);
  }
  async search() {
    ...
    if (t) {
      ...
      try {
        ...
        e ? (this.setState({
          ...
          currentPage: 1,
          hasMoreData: !1
        }), this.opts.toast.success(this.opts.t(o.SEARCH_SUCCESS)), A("live_search", "search", !0, t)) : (this.setState({
          lives: [],
          currentPage: 1,
          hasMoreData: !1
        }), this.opts.toast.error(this.opts.t(o.LIVE_NOT_FOUND)), A("live_search", "search", !1, t));
      } catch (e) {
        if (F.error("搜索直播失败", "", e), A("live_search", "search", !1), e instanceof v)
          this.opts.toast.error(X(
            e.code ?? 0,
            e.message,
  ...
  async clearSearch() {
    this.pageCursors = w(), this.setState({
      searchInput: "",
      currentPage: 1,
      hasMoreData: !0
  ...
  async showDetail(t) {
    ...
    const { snapshot: e, toastError: i } = await ss(t, this.opts.t);
  ...
  async confirmDelete() {
    ...
    if (t)
      try {
        await this.opts.endLive(t), this.opts.toast.success(this.opts.t(o.DISSOLVE_SUCCESS)), A("live_crud", "delete", !0, t);
        const e = oe({
          currentPage: this.state.currentPage,
          hasMoreData: this.state.hasMoreData,
        ...
        F.error("解散直播失败", "", e), A("live_crud", "delete", !1, t), this.opts.toast.error(this.opts.t(o.DISSOLVE_FAILED));
      } finally {
...
const is = y("GiftConfig"), rs = { name: "", description: "" };
class Hi {
  constructor(t) {
    this.opts = t, this.state = {
      loading: !1,
  ...
  async fetchGiftList() {
    ...
    try {
      ...
      is.error("fetchGiftConfig", String(this.opts.t(o.GET_GIFT_LIST_FAILED)), t);
  ...
  goToCategoryManagement() {
    Qe(this.state.categoryList), this.opts.onNavigateToCategoryManagement();
  }
  async copyGiftId(t) {
    await H(String(t || "")), this.opts.toast.success(this.opts.t(o.COPY_LABEL_COPIED, { label: this.opts.t(o.GIFT_ID) }));
  }
      ...
      try {
        ...
        for (const c of a)
          if (c.status === "fulfilled") {
            const { langKey: l, result: h } = c.value;
  ...
  async openLangEditDialog(t, e) {
    const i = this.state.giftList.find((n) => n.id === t), r = Xe(i?.languageList, e);
    let a = r.form;
    if (r.needsFetch)
      try {
  ...
  async clearLang(t, e) {
    try {
      await this.opts.actions.deleteGiftLanguage({ giftId: t, language: m[e].code }), this.opts.toast.success(this.opts.t(o.LANGUAGE_CONFIG_CLEARED)), this.setState({
        giftLangConfig: {
          ...this.state.giftLangConfig,
          [e]: { name: "", description: "" }
        }
...
class Wi {
  ...
  async fetchCategoryList(t = !0) {
    ...
    try {
      ...
      as.error("fetchCategoryList", String(this.opts.t(o.GET_CATEGORY_LIST_FAILED)), e), this.opts.toast.error(this.formatError(e, this.opts.t(o.GET_CATEGORY_LIST_FAILED))), this.setState({ categoryList: [] });
    } finally {
      this.setState({ loading: !1 });
    }
  }
   ...
   */
  onLanguageChanged() {
    this.state.categoryList.length !== 0 && this.setState({ categoryList: es(this.state.categoryList) });
  }
  // -------- 导航 / 复制 --------
  goBack() {
  ...
  async copyCategoryId(t) {
    await H(String(t || "")), this.opts.toast.success(
  ...
  // -------- 单语言编辑子弹窗 --------
  async openLangEditDialog(t, e) {
    const i = this.state.categoryList.find((c) => c.id === t), r = m[e].code, a = i?.languageList?.find((c) => U(c).langCode === r);
    let n = { ...nt };
    if (a) {
      const c = U(a);
  ...
  async clearLang(t, e) {
    try {
      await this.opts.actions.deleteGiftCategoryLanguage(t, m[e].code), this.opts.toast.success(this.opts.t(o.LANGUAGE_CONFIG_CLEARED)), this.setState({
        categoryLangConfig: {
          ...this.state.categoryLangConfig,
          [e]: { name: "", description: "" }
        }
