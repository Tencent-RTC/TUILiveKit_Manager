...
function Oo() {
  return mi || (mi = 1, (function(E, T) {
    ((O, z) => {
      ...
      var O, z;
      function G(e) {
        this.name = "__st" + (1e9 * Math.random() >>> 0) + O + "__", e?.forEach(this.add, this), O += 1;
      }
      ...
      } }), window.WeakSet || (O = Date.now() % 1e9, G.prototype.add = function(e) {
        var t = this.name;
...
`), Y = (q.payload = l?.body, q.ret = L, q.isErr = +N, q.from = f || n.getFromParam(t), { msg: Y, code: L, trace: v, errorMsg: "", from: f || n.getFromParam(t), originFrom: p, level: R.API_RESPONSE }), U = e.slowApiLog && mr(q?.duration, e, q);
              Y.level = U ? R.SLOW_NET_REQUEST : gr(b, N), (U || Y.level !== R.API_RESPONSE || t.isWhiteList || (U = (U = t.config) == null ? void 0 : U.api) != null && U.reportRequest) && n.publishNormalLog(Y, t), n.publishSpeed(q, t);
            }, 0);
          }).catch(function(g) {
...
`).reduce(function(U, D) {
                ...
                }, {}), k = at(N, F, "res"), I = cn(o.aegisXhrReqHeader), F = (N = e.api) == null ? void 0 : N.apiDetail, q = F ? it(a, (N = e.api) == null ? void 0 : N.reqParamHandler, { url: w }) : "", Y = F ? it(o.response, (N = e.api) == null ? void 0 : N.resBodyHandler, { url: w }) : "";
                try {
                  ((U, D, j, J) => {
                    var Ce, Ye, ee;
                    try {
...
It is recommended that you contact us for feedback and thank you for your support.`, "color: red"), s.sendSDKError(f);
      ...
      var ai = function(r) {
        ...
        var t, n, r, i, o, a, s, u, l, c, f, p, d, h, g, m, b, y, w, v, L, N, k, I, F, q, Y, U, D, j, J, Ce, Ye, ee, pt, te, ke, Ze, Dn, zt = this;
        document.elementFromPoint && (this.pendingTimeouts.forEach(function(W) {
          ...
          return K(zt, void 0, void 0, function() {
            var W, X, Ae, ye, ci, fi, Gt, ht, De, Hn, et, gt, Vt, ue, $t, ce, jn, di, oe, qn, Wn, Jt, Xt, pi, Ie, mt, vt, yt, wt, Kt, Bn, tt, Eo = this;
            return se(this, function(bt) {
              switch (bt.label) {
                case 0:
                  if (W = window.innerHeight, X = window.innerWidth, Ae = c.x, ye = c.y, X <= Ae || W <= ye) return [2];
                  for (ci = Ae + (X - Ae) / 2, fi = ye + (W - ye) / 2, Gt = [], ht = i + 1, Hn = De = 0, et = 1; et < ht; et++) ue = ye + (W - ye) * et / ht, ce = Bt(gt = Ae + (X - Ae) * et / ht, fi), Vt = Bt(ci, ue), $t = Bt(gt, ue), gt = Bt(gt, W - ue), ue = Wt(ce, a, s, u, l), ue.isWhitePoint && (De += 1), Gt.push(ue.selectorPointers), et !== ht / 2 && (ce = Wt(Vt, a, s, u, l), ue = ce.isWhitePoint, Vt = ce.selectorPointers, ue && (De += 1), ce = Wt($t, a, s, u, l), ue = ce.isWhitePoint, $t = ce.selectorPointers, ue && (De += 1), ce = Wt(gt, a, s, u, l), ce.isWhitePoint && (De += 1), Gt.push(Vt, $t, ce.selectorPointers));
                  return vt = v <= De, yt = !1, jn = { samePointerMap: {}, maximumSamePointer: {} }, d || (oe = Gt.map(function(B) {
                    return B.slice(0, h);
                  }), Qt = oe.map(function(B) {
                    return B.join(" < ");
                  }).reduce(function(B, ne) {
                    return B[ne] = B[ne] ? B[ne] + 1 : 1, B;
                  }, {}), zn = Object.keys(Qt).map(function(B) {
                    return [B, Qt[B]];
                  }).sort(function(B, ne) {
                    return ne[1] - B[1];
                  })[0], hi = zn[0], oe = { samePointerMap: Qt, maximumSamePointerClass: hi, maximumSamePointerCounter: zn[1] }, jn = { samePointerMap: oe.samePointerMap, maximumSamePointer: ((tt = {})[di = oe.maximumSamePointerClass] = oe = oe.maximumSamePointerCounter, tt) }, Hn = oe, tt = yo.some(function(B) {
                    return di.startsWith(B);
...
linkError: ` + Ie.linkMsg : ""), level: R.BLANK_SCREEN, errorMsg: B || null, originFrom: e.getOriginFrom() }, e.normalLogPipeline(ne);
          ...
          var W;
          k || zt.isAwaitingClientScreenshot || N === Ke.ERROR && !t || (N = t ? Ke.ERROR : Ke.DOM_CHANGE, clearTimeout(n), W = ii(e).debounceDuration, n = setTimeout(function() {
            return K(zt, void 0, void 0, function() {
              return se(this, function(X) {
                switch (X.label) {
                  case 0:
          ...
          (j = new MutationObserver(function(W) {
            var X = ai(W[0].target), X = f.includes(X.selector);
            W.length === 1 && X || U();
...
function Po(E, T) {
  const O = Ri({
    ...E,
    ...T,
    credentials: T.credentials === void 0 ? E.credentials : T.credentials ? { ...T.credentials } : null
  });
  if (T.credentials !== void 0) {
    const z = O.credentials;
    O.sdkAppId = z?.sdkAppId ?? 0, O.userId = z?.userId ?? "", O.userSig = z?.userSig ?? "";
  }
  return O;
}
let Yt = Ei();
...
function Ho(E) {
  const T = Me.getState();
  Me = E ?? Ti, Me.reset(), Me.setState(T);
}
function Oi() {
  return Me.getState();
}
...
function xi(E, T) {
  if (!T)
    return !1;
  const O = E;
  return typeof O.setConfig == "function" ? (O.setConfig({ uin: T }), !0) : typeof O.setUin == "function" ? (O.setUin(T), !0) : O.config && typeof O.config == "object" ? (O.config.uin = T, !0) : (console.warn("[Aegis] 当前 SDK 实例不支持动态更新 uin"), !1);
}
function Wo(E) {
  if (!E.id)
...
function Ci(E, T) {
  ...
  if (O)
    try {
      O.reportTime?.({ name: E, duration: T });
    } catch {
    }
}
function ki(E, T, O, z) {
  nt(E, T, O ? "success" : "fail", z);
}
function ko(E, T) {
  const O = Date.now();
  return T().finally(() => {
...
function Ao(E = Co) {
  ...
  if (!T) return;
  const O = `${T}:${E}`;
  vi.has(O) || (vi.add(O), nt("app_init", "live_manager", E));
}
...
function Io(E) {
  ...
  if (!T) return;
  const O = `${T}:${E}`;
  yi.has(O) || (yi.add(O), nt("capability", E, "on"));
}
...
function Mo(E) {
  ...
  if (!T) return;
  const O = `${T}:${E}`;
  wi.has(O) || (wi.add(O), nt("feature_use", E));
}
let bi = null;
function No(E) {
  if (E === bi) return;
...
function Uo(E) {
  ...
  if (E instanceof Error) {
    const T = [];
    if (E.message && T.push(E.message), E.stack) {
      const O = E.stack.split(`
`)[1]?.trim();
      O && T.push(O);
    }
    return T.join(" | ");
...
class Ai {
   ...
   */
  debug(T, ...O) {
    this.log("debug", T, O[0], O.length > 1 ? O.slice(1) : void 0);
  }
  /**
   * 信息日志
   */
  info(T, ...O) {
    this.log("info", T, O[0], O.length > 1 ? O.slice(1) : void 0);
  }
  /**
   * 警告日志
   */
  warn(T, ...O) {
    this.log("warn", T, O[0], O.length > 1 ? O.slice(1) : void 0);
  }
  /**
   * 错误日志 - 同时触发 RUM 上报（如启用）
   */
  error(T, ...O) {
    const z = O[0], G = O.length > 1 ? O.slice(1) : void 0;
    if (this.log("error", T, z, G), this.config.enableReport && G) {
      const ae = G.find((x) => x instanceof Error) ?? G[G.length - 1];
      ae && this.reportToRum(T, z, ae);
    }
  }
  /**
   * 统一日志输出
   */
  log(T, O, z, G) {
    if (Zt[T] < Zt[Vn] || this.config.minLevel && Zt[T] < Zt[this.config.minLevel] || !this.config.enableConsole) return;
    const ae = this.formatTimestamp(), x = this.config.prefix ? `[${this.config.prefix}]` : "", K = `${ae}${x}[${O}] ${z}`;
    switch (T) {
      case "debug":
        console.debug(K, G ?? "");
        break;
   ...
   */
  formatTimestamp() {
    const T = /* @__PURE__ */ new Date(), O = String(T.getHours()).padStart(2, "0"), z = String(T.getMinutes()).padStart(2, "0"), G = String(T.getSeconds()).padStart(2, "0"), ae = String(T.getMilliseconds()).padStart(3, "0");
    return `[${O}:${z}:${G}.${ae}]`;
  }
  /**
   * 上报错误到 RUM (Aegis)
