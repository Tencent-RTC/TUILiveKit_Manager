import { ref as i, onUnmounted as D, onMounted as G } from "vue";
import { as as ae, au as oe, br as ne, bJ as ie, bI as se, u as le, m as ue, R as ce, Q as de, p as ve } from "./main-layout.DLWT3EVA.js";
import { c as W, n as ge, j as V, h as o } from "./logger.D3BLOWQd.js";
import "./gift.DGa9OHLZ.js";
import { aM as O, aN as fe, aR as me, aH as he, I as Me, ah as Le, aP as be, ag as we, aO as q, az as H, Y as pe, aQ as ye } from "./layout.B11gjrzt.js";
import { s as Ce, t as Se } from "./utils.CkhUhDme.js";
import { C as Pe } from "./chat-state.DcgAjEAj.js";
const c = W("RiskControl");
function qe(s) {
  const { liveId: a, pageSize: g } = s, f = i("cloud"), y = i(!1), C = i(0), b = i(!0), w = i([]), _ = i(0), T = i(1), S = i(!1), r = i([]), d = i([]), m = i(!1), v = i(null);
  let M = !0;
  ge("risk_control");
  const J = () => {
  ...
  O().then((e) => {
    M && (f.value = e, J(), e === "custom" && fe().then((t) => {
      M && (b.value = t.Enabled);
    }).catch(() => {
    }));
  }), D(() => {
  ...
  const Y = async () => {
    ...
    try {
      let t;
      return f.value === "custom" ? t = await q({ pageSize: g, liveId: a, ...e }) : t = await H({ pageSize: g, liveId: a, ...e }), M && (w.value = t.list || [], _.value = t.total || 0, T.value = e.pageNum || 1), t;
    } catch (t) {
    ...
    try {
      ...
      if (!P) {
        ...
        t = N.map((h) => h.id), P = N.map(({ id: h, content: L, userId: E }) => ({ id: h, content: L, userId: E }));
      }
      if (f.value === "custom") {
    ...
    try {
      await O() === "custom" ? await me(e) : await he(e), o("moderation", "delete", !0, String(e.length));
    } catch (t) {
  ...
  return {
    ...
    sendViolationWarning: async () => {
      if (!a) throw new Error("liveId is required");
      try {
        const e = await ne(a, "default", Me.VIOLATION_SUGGESTION_DEFAULT);
...
const u = W("LiveMonitor"), He = {
  LIVE_LIST_CHANGED: "LIVE_LIST_CHANGED"
};
let n = null, ke = !0;
const R = i([]), K = i(!0), B = i(null), Ie = i({}), _e = i(null);
let l = null;
const I = i([]), U = i(1), $ = i(!0), z = i(!1);
function F() {
  return l || (l = new de({ pageSize: 8 }), l.subscribe(() => {
    const s = l.getSnapshot();
    I.value = s.list, U.value = s.currentPage, $.value = s.hasMoreData, z.value = s.loading, R.value = [...s.list];
  }), l);
}
function Ke() {
  const s = () => {
  ...
  return D(() => {
    ...
    if (s(), F(), l) {
      ...
      I.value = r.list, U.value = r.currentPage, $.value = r.hasMoreData, z.value = r.loading;
    }
  }), n || s(), F(), {
    init: (r) => {
