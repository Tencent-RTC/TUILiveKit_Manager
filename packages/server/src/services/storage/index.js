...
/**
 * 注册存储 Provider 类型，供配置项 STORAGE_PROVIDER 按名称选择。
 * @param {string} name
 * @param {new () => import('./StorageProvider')} ProviderClass
 */
function registerStorageProvider(name, ProviderClass) {
  if (!name || typeof name !== 'string') {
    throw new Error('Storage Provider name must be a non-empty string');
  }
  if (typeof ProviderClass !== 'function') {
    throw new Error(`Storage Provider "${name}" must be a constructor`);
  }
  PROVIDER_MAP[name.toLowerCase()] = ProviderClass;
  activeProvider = null;
}

/**
 * 直接注入当前活动的存储 Provider 实例，供客户服务端插件覆盖默认存储能力。
 * @param {import('./StorageProvider')} provider
 */
function setStorageProvider(provider) {
  if (!provider || typeof provider !== 'object') {
    throw new Error('Storage Provider instance must be an object');
  }
  if (typeof provider.isEnabled !== 'function' || typeof provider.uploadFile !== 'function') {
    throw new Error('Storage Provider must implement isEnabled() and uploadFile()');
  }
