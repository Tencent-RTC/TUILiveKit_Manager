...
// 2.1 若 .env 不存在，用 example.env 生成（开箱即用）
const exampleEnv = resolve(deployDir, 'config', 'example.env');
const envPath = resolve(deployDir, 'config', '.env');
if (existsSync(exampleEnv) && !existsSync(envPath)) {
  cpSync(exampleEnv, envPath);
}
